...
class ChatState {
  const ChatState(
      ...
      this.persona = const UserPersona(),
      this.sessionId = 'luna-main',
...
class ChatController extends Notifier<ChatState> {
  ...
  late final CharacterProfile _luna = const CharacterProfile(
      id: 'luna',
      name: 'Luna',
      subtitle: '月光下的旅人',
      emoji: '🌙',
      description: '一位在旧城边缘旅行的神秘旅人。她相信每一盏灯都替某个人保留着回家的方向。');
  late final CharacterProfile _aria = const CharacterProfile(
      id: 'aria',
      name: 'Aria',
      subtitle: '星海观测者',
      emoji: '🦋',
      description: '来自远方观测站的记录员，习惯把每段相遇写进星图。');
  late final CharacterProfile _nova = const CharacterProfile(
      id: 'nova',
      name: 'Nova',
      subtitle: '温柔的实验助手',
      emoji: '🤖',
  ...
  List<ChatMessage> _lunaMessages() => [
        ChatMessage(
            ...
            content: '夜色刚刚降临，月光像一层薄纱落在古城的屋檐上。Luna 靠在窗边，回头看向你。',
            createdAt: DateTime.now()),
        ChatMessage(
            id: _uuid.v4(),
            role: MessageRole.assistant,
            content: '“如果明天就要出发，你会带上什么？”她轻声问道，指尖轻轻敲着窗框。',
            createdAt: DateTime.now()),
        ChatMessage(
            id: _uuid.v4(),
            role: MessageRole.user,
            content: '我会带上一盏灯。不是为了照亮路，而是想让迷路的人知道，这里还有人在等他们。',
            createdAt: DateTime.now()),
      ];
  ...
  CharacterProfile _profileFromCard(CharacterCard card) => CharacterProfile(
      id: card.id,
      name: card.name,
  ...
  CharacterProfile _characterById(String id) => _characters()
      .firstWhere((character) => character.id == id, orElse: () => _luna);
  ...
  Future<void> _loadSessions() async {
    ...
    state = state.copyWith(
        ...
        character: _characterById(current.characterId),
        messages: current.messages,
        clearError: true);
  }

  void _persistSessions() {
    LocalJsonStore.write('chat_sessions',
        {'sessions': state.sessions.map(_sessionJson).toList()});
  }

  Map<String, dynamic> _sessionJson(ChatSessionSummary session) => {
        'id': session.id,
  ...
  ChatSessionSummary _sessionFromJson(
      ...
      ChatSessionSummary(
          id: value['id'] as String? ?? _uuid.v4(),
          characterId: value['characterId'] as String? ?? 'luna',
          title: value['title'] as String? ?? '新会话',
          updatedAt: DateTime.tryParse(value['updatedAt'] as String? ?? '') ??
              DateTime.now(),
          preview: value['preview'] as String? ?? '',
          messages: value['messages'] is List
              ? (value['messages'] as List)
                  .whereType<Map>()
                  .map((message) => ChatMessage(
                      id: message['id'] as String? ?? _uuid.v4(),
                      role:
                          MessageRole.values
                      ...
                      status: MessageStatus.values.firstWhere(
                          (status) => status.name == message['status'],
  ...
  void selectSession(String id) {
    ...
    final messages =
        session.messages.isEmpty ? _lunaMessages() : session.messages;
    state = state.copyWith(
        sessionId: id,
        character: _characterById(session.characterId),
        messages: messages,
        swipeIndex: 0,
        swipeCount: 1,
        clearError: true);
  ...
  void selectCharacter(CharacterProfile character) {
    ...
    final messages = greeting.isEmpty
        ? state.messages
        : [
            ChatMessage(
                id: _uuid.v4(),
  ...
  void updatePersona(UserPersona persona) =>
  ...
  void selectGreeting(String greeting) {
    ...
    final firstAssistant = state.messages
        .indexWhere((message) => message.role == MessageRole.assistant);
    if (firstAssistant < 0) return;
    final messages = [...state.messages];
    messages[firstAssistant] =
        messages[firstAssistant].copyWith(content: greeting.trim());
    ...
    state = state.copyWith(
        sessions: state.sessions
            .map((session) => session.id == id
                ? session.copyWith(title: clean, updatedAt: DateTime.now())
                : session)
            .toList());
    _persistSessions();
  }
  ...
  void purgeCharacterSessions(String characterId) {
    ...
    final currentRemoved = state.sessions.any((session) =>
        session.id == state.sessionId && session.characterId == characterId);
    if (currentRemoved) {
      final next = remaining.first;
      state = state.copyWith(
          sessions: remaining,
          sessionId: next.id,
          character: _characterById(next.characterId),
  ...
  void _saveCurrentSession() {
    final updated = state.sessions
        .map((session) => session.id == state.sessionId
            ? session.copyWith(
                updatedAt: DateTime.now(),
                preview: state.messages.isEmpty
                    ? '还没有消息'
  ...
  void appendAssistantDelta(String delta) {
    if (state.messages.isNotEmpty &&
        ...
        state.messages.last.status == MessageStatus.streaming) {
  ...
  void finishAssistant() {
    ...
    state = state.copyWith(messages: [
      ...state.messages.take(state.messages.length - 1),
      last.copyWith(status: MessageStatus.complete)
    ], isGenerating: false, swipeIndex: 0, swipeCount: 1);
    _saveCurrentSession();
  }
  ...
  void swipeLastAssistant() {
    ...
    const alternatives = [
      'Luna 没有立刻回答。她把那盏灯接过去，掌心护住微弱的火光。“这样的话，天亮以前我们都不会走散。”',
      '她望向远处的城墙，轻轻点头。“那就出发吧。灯光会替我们记住来时的路。”'
    ];
    final nextIndex = state.swipeCount % alternatives.length;
    state = state.copyWith(messages: [
      ...state.messages.take(state.messages.length - 1),
      current.copyWith(
          content: alternatives[nextIndex], status: MessageStatus.complete)
    ], swipeIndex: nextIndex + 1, swipeCount: state.swipeCount + 1);
  }
