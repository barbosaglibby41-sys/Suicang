...
class ChatContextBuilder {
  ...
  static GenerationRequest build({
    ...
    final system =
        _systemPrompt(character, persona, preset, worldBooks, history);
  ...
  static String _systemPrompt(CharacterProfile character, UserPersona persona,
      PromptPreset? preset, List<WorldBook> books, List<ChatMessage> history) {
    final recentText = history.reversed
        .take(8)
    ...
    for (final book in books) {
      for (final entry in book.entries) {
        ...
        final hit = entry.keys.any((key) =>
            key.trim().isNotEmpty && recentText.contains(key.toLowerCase()));
    ...
    final systemPrompt =
        card?.systemPrompt.isNotEmpty == true ? card!.systemPrompt : '';
    final description = card?.description.isNotEmpty == true
        ? card!.description
        : character.description;
    final personality =
        card?.personality.isNotEmpty == true ? card!.personality : '';
    final scenario = card?.scenario.isNotEmpty == true ? card!.scenario : '';
    final postHistory = card?.postHistoryInstructions.isNotEmpty == true
    ...
    final sections = <String>[
      ...
      if (persona.name.isNotEmpty || persona.description.isNotEmpty)
        '用户 Persona：\n${persona.name}\n${persona.description}',
