...
class OpenAiCompatibleProvider implements LlmProvider {
  OpenAiCompatibleProvider(
      {required this.baseUrl, required this.apiKey, Dio? dio})
      : _dio = dio ??
            Dio(BaseOptions(
  ...
  Future<String> testConnection({String? model}) async {
    try {
      ...
      if (response.statusCode != null &&
        ...
        return '连接成功${model == null || model.isEmpty ? '' : ' · $model'}';
  ...
  Stream<GenerationEvent> generate(GenerationRequest request) async* {
    try {
      ...
      if (stream == null) {
        yield const GenerationFailed('模型没有返回数据');
        return;
      }
      var buffer = '';
      await for (final bytes in stream) {
        ...
        for (final line in lines) {
          ...
          if (payload == '[DONE]') {
            yield const GenerationCompleted();
            continue;
          }
          try {
            ...
            final delta =
                first is Map ? ((first['delta'] as Map?)?['content']) : null;
