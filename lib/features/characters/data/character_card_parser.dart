...
class CharacterCardParser {
  ...
  static CharacterCard fromJsonText(String source, {String sourceName = '导入角色卡', String? avatarData}) {
    final decoded = jsonDecode(source);
    if (decoded is! Map<String, dynamic>) throw const FormatException('角色卡 JSON 根节点必须是对象');
    return fromJson(decoded, sourceName: sourceName, avatarData: avatarData);
  }

  static CharacterCard fromPng(Uint8List bytes, {String sourceName = 'PNG 社区角色卡'}) {
    const signature = [137, 80, 78, 71, 13, 10, 26, 10];
    if (bytes.length < 8 || !_startsWith(bytes, signature)) throw const FormatException('不是有效的 PNG 文件');
    var offset = 8;
    while (offset + 12 <= bytes.length) {
      ...
      if (type == 'tEXt') {
        ...
        if (split > 0) {
          ...
          final value = latin1.decode(data.sublist(split + 1), allowInvalid: true);
          if (keyword == 'chara') return fromJsonText(utf8.decode(base64.decode(value.trim())), sourceName: sourceName, avatarData: base64Encode(bytes));
        }
      } else if (type == 'iTXt') {
        ...
        if (zero > 0 && ascii.decode(data.sublist(0, zero)) == 'chara') {
          ...
          if (textStart != null) {
            ...
            return fromJsonText(utf8.decode(base64.decode(value.trim())), sourceName: sourceName, avatarData: base64Encode(bytes));
          }
        }
      }
      offset = dataEnd + 4;
  ...
  static CharacterCard fromJson(Map<String, dynamic> json, {String sourceName = '导入角色卡', String? avatarData}) {
    ...
    return CharacterCard(
      ...
      firstMessage: _string(data, 'first_mes', fallback: _string(data, 'first_message')),
      exampleMessages: _string(data, 'mes_example', fallback: _string(data, 'example_messages')),
      ...
      extensions: data['extensions'] is Map ? Map<String, dynamic>.from(data['extensions'] as Map) : <String, dynamic>{},
      source: sourceName,
      tags: _strings(data, 'tags'),
    );
  }

  static String _string(Map<String, dynamic> map, String key, {String fallback = ''}) => map[key] is String && (map[key] as String).trim().isNotEmpty ? map[key] as String : fallback;

  static List<String> _strings(Map<String, dynamic> map, String key) => map[key] is List ? (map[key] as List).whereType<String>().where((item) => item.trim().isNotEmpty).toList() : const [];
  ...
  static CharacterBook? _book(dynamic value) {
    ...
    return CharacterBook(name: value['name'] is String ? value['name'] as String : 'Character Book', entries: entries.whereType<Map>().map((entry) => WorldBookEntry(id: '${entry['uid'] ?? entry['id'] ?? entries.indexOf(entry)}', keys: entry['keys'] is List ? (entry['keys'] as List).whereType<String>().toList() : const [], content: entry['content'] is String ? entry['content'] as String : '', constant: entry['constant'] == true, selective: entry['selective'] != false, enabled: entry['enabled'] != false, position: entry['position'] is String ? entry['position'] as String : 'before_char')).toList());
  }
