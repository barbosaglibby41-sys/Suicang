...
class _CharactersScreenState extends ConsumerState<CharactersScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final visible = characters.where((character) {
      final matchesQuery = query.isEmpty ||
          ...
          character.tagline.toLowerCase().contains(query);
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          SliverToBoxAdapter(
              child: _CharactersHero(onImport: _showImportOptions)),
          SliverPadding(
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
                ...
                title: const Text('PNG 角色卡'),
                subtitle: const Text('读取 Tavern Card V1 / V2 元数据'),
                onTap: () {
                  Navigator.pop(context);
                  _pickCharacterFile(['png']);
                }),
            ListTile(
                leading: const Icon(Icons.data_object),
                title: const Text('JSON 角色卡'),
                subtitle: const Text('兼容 character card JSON 格式'),
                onTap: () {
                  Navigator.pop(context);
                  _pickCharacterFile(['json']);
                }),
            ListTile(
                leading: const Icon(Icons.menu_book_outlined),
                title: const Text('世界书 / Lorebook'),
                subtitle: const Text('导入 SillyTavern 世界书条目'),
                onTap: () {
                  Navigator.pop(context);
                  _pickWorldBook();
                }),
  ...
  Future<void> _pickCharacterFile(List<String> extensions) async {
    ...
    try {
      ...
      final card = extensions.first == 'png'
          ? CharacterCardParser.fromPng(bytes, sourceName: file.name)
          : CharacterCardParser.fromJsonText(utf8.decode(bytes),
              sourceName: file.name);
      ref.read(characterLibraryProvider.notifier).upsert(card);
      if (mounted)
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('已导入「${card.name}」角色卡')));
    } catch (error) {
      if (mounted)
        ScaffoldMessenger.of(context)
    ...
    try {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content:
                Text('已读取世界书「${book.name}」，包含 ${book.entries.length} 个条目')));
    } catch (error) {
      if (mounted)
        ScaffoldMessenger.of(context)
  ...
  void _openCard(CharacterCard character) {
    showModalBottomSheet<void>(
        ...
        showDragHandle: true,
        builder: (_) => _CharacterDetail(
            card: character,
            onSave: (next) =>
                ref.read(characterLibraryProvider.notifier).upsert(next),
            onDelete: () {
              ref
                  .read(chatControllerProvider.notifier)
                  .purgeCharacterSessions(character.id);
              ref.read(characterLibraryProvider.notifier).remove(character.id);
            }));
  }
}
  ...
  Widget build(BuildContext context) => Container(
      ...
      child: SafeArea(
          ...
          child:
            ...
            Row(children: [
              const Expanded(
                  child: Column(
                    ...
                    SizedBox(height: 5),
                    Text('导入社区角色，保留完整 SillyTavern 能力',
            ...
            Row(children: [
              _HeroStat(value: '12', label: '角色卡'),
              _HeroStat(value: '08', label: '最近使用'),
              _HeroStat(value: '04', label: '世界书')
            ])
          ])));
}
  ...
  Widget build(BuildContext context) => TextField(
      ...
      decoration: InputDecoration(
          ...
          border: OutlineInputBorder(
              ...
              borderSide: BorderSide(color: SuicangTheme.line))));
}
              ...
              children: [
                Container(
                  ...
                  child: character.avatarData == null
                      ? Center(
                          child: Text(character.avatar,
                              style: const TextStyle(fontSize: 44)))
                      : ClipRRect(
                          borderRadius: BorderRadius.circular(16),
                          child: Image.memory(
                              base64Decode(character.avatarData!),
                              fit: BoxFit.cover),
                        ),
                ),
                const SizedBox(height: 12),
...
class _CharacterDetail extends StatefulWidget {
  const _CharacterDetail(
      {required this.card, required this.onSave, required this.onDelete});
...
class _CharacterDetailState extends State<_CharacterDetail> {
  ...
  late final _personality =
      TextEditingController(text: widget.card.personality);
  late final _scenario = TextEditingController(text: widget.card.scenario);
  late final _firstMessage =
      TextEditingController(text: widget.card.firstMessage);
  late final _exampleMessages =
      TextEditingController(text: widget.card.exampleMessages);
  late final _systemPrompt =
      TextEditingController(text: widget.card.systemPrompt);
  late final _postHistory =
      TextEditingController(text: widget.card.postHistoryInstructions);
  late final _creatorNotes =
      TextEditingController(text: widget.card.creatorNotes);
          ...
          child: Column(
            ...
            children: [
              ...
              _EditorSection(
                  ...
                  title: '基础资料',
                  subtitle: '角色在工作区中显示的身份信息',
                  children: [
                    _Field(controller: _name, label: '角色名称'),
                    _Field(controller: _tagline, label: '副标题'),
                    _Field(controller: _description, label: '角色描述', maxLines: 4)
                  ]),
              _EditorSection(
                  icon: Icons.psychology_outlined,
                  title: '角色行为',
                  subtitle: '这些字段会进入生成上下文',
                  children: [
                    _Field(
                        ...
                        maxLines: 3),
                    _Field(
                        controller: _scenario, label: 'Scenario', maxLines: 3),
                    _Field(
                        controller: _systemPrompt,
                        label: 'System Prompt',
                        maxLines: 4),
                    _Field(
                        controller: _postHistory,
              ...
              _EditorSection(
                  ...
                  title: '开场体验',
                  subtitle:
                      '兼容 First Message、Example Messages 和 Alternate Greetings',
                  children: [
                    _Field(
                    ...
                    _GreetingEditor(
                        ...
                        onChanged: (value) =>
                            setState(() => _greetings = value))
                  ]),
              _EditorSection(
                icon: Icons.menu_book_outlined,
                title: 'Character Book',
                subtitle: '可按关键词注入的角色专属世界书',
                children: [
                  Row(
                    children: [
                      Expanded(
                          child: Text('${_entries.length} 个条目',
                              style: const TextStyle(
                                  fontWeight: FontWeight.w700))),
                      OutlinedButton.icon(
                          onPressed: _addEntry,
                  ...
                  ReorderableListView.builder(
                    ...
                    itemBuilder: (_, index) => _BookEntryEditor(
  ...
  Future<void> _export() async {
    ...
    final jsonFile =
        await LocalJsonStore.writeText('export_${widget.card.id}', json);
    var message = 'JSON 已复制，并保存到应用文档导出目录';
    try {
      final png = CharacterCardExporter.toPngCard(widget.card);
      await LocalJsonStore.writeBytes('export_${widget.card.id}', png,
          extension: 'png');
  ...
  void _addEntry() => setState(() => _entries.add(WorldBookEntry(
      id: DateTime.now().microsecondsSinceEpoch.toString(),
      keys: const [],
      content: '')));

  void _save() {
    final book = _entries.isEmpty
        ? null
        : CharacterBook(
            name: widget.card.characterBook?.name ?? 'Character Book',
            entries: _entries);
    widget.onSave(widget.card.copyWith(
        ...
        alternateGreetings: _greetings,
        characterBook: book,
        clearCharacterBook: _entries.isEmpty));
    Navigator.pop(context);
  }
}
...
class _BookEntryEditor extends StatefulWidget {
  const _BookEntryEditor(
      {required this.entry, required this.onChanged, required this.onDelete});
...
class _BookEntryEditorState extends State<_BookEntryEditor> {
  ...
  WorldBookEntry _updated(
      ...
      WorldBookEntry(
          ...
          enabled: enabled ?? widget.entry.enabled,
          ...
          child: Column(children: [
            Row(children: [
              ...
              Switch(
                  ...
                  onChanged: (value) =>
                      widget.onChanged(_updated(enabled: value))),
              IconButton(
                  tooltip: '删除条目',
            ...
            TextField(
                ...
                onChanged: (value) => widget.onChanged(_updated(
                    keys: value
                        .split(',')
                        .map((item) => item.trim())
            ...
            TextField(
                controller: _content,
                maxLines: 3,
                decoration: const InputDecoration(labelText: '注入内容'),
                onChanged: (value) =>
                    widget.onChanged(_updated(content: value))),
            SwitchListTile(
                contentPadding: EdgeInsets.zero,
                dense: true,
                title: const Text('常驻注入', style: TextStyle(fontSize: 12)),
                subtitle:
                    const Text('无论关键词是否命中都注入', style: TextStyle(fontSize: 10)),
                value: widget.entry.constant,
                onChanged: (value) =>
                    widget.onChanged(_updated(constant: value))),
            SwitchListTile(
                contentPadding: EdgeInsets.zero,
                dense: true,
                title: const Text('选择性触发', style: TextStyle(fontSize: 12)),
                value: widget.entry.selective,
                onChanged: (value) =>
                    widget.onChanged(_updated(selective: value))),
            DropdownButtonFormField<String>(
                value: widget.entry.position,
                decoration: const InputDecoration(labelText: '注入位置'),
                items: const [
                  DropdownMenuItem(value: 'before_char', child: Text('角色定义之前')),
                  DropdownMenuItem(value: 'after_char', child: Text('角色定义之后')),
                  DropdownMenuItem(
                      value: 'before_example_messages', child: Text('示例消息之前')),
                  DropdownMenuItem(
                      value: 'after_example_messages', child: Text('示例消息之后'))
                ],
                onChanged: (value) {
                  if (value != null)
                    widget.onChanged(_updated(position: value));
                })
          ])));
}
...
class _DetailSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(content,
            style: const TextStyle(
                fontSize: 13, height: 1.5, color: SuicangTheme.muted))
      ]));
}
        ...
        child: Row(
          children: [
            Container(
              ...
              child: card.avatarData == null
                  ? Center(
                      child: Text(card.avatar,
                          style: const TextStyle(fontSize: 31)))
                  : ClipRRect(
                      borderRadius: BorderRadius.circular(18),
                      child: Image.memory(base64Decode(card.avatarData!),
                          fit: BoxFit.cover)),
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    const Text('SillyTavern Card Compatible',
                        style: TextStyle(
          ...
          children: [
            Row(children: [
              Container(
                  ...
                  child: Icon(icon, size: 17, color: SuicangTheme.primary)),
...
class _GreetingEditor extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
        ...
        children: [
          Row(children: [
            const Expanded(
                child: Text('Alternate Greetings',
                    style:
                        TextStyle(fontSize: 12, fontWeight: FontWeight.w700))),
            Text('${greetings.length} 条',
                style:
                    const TextStyle(fontSize: 10, color: SuicangTheme.muted)),
          ]),
          const SizedBox(height: 7),
          ReorderableListView.builder(
            shrinkWrap: true,
            ...
            itemBuilder: (_, index) => Container(
              key: ValueKey('greeting-$index-${greetings[index]}'),
              ...
              child:
                ...
                const Icon(Icons.drag_indicator,
                    size: 17, color: SuicangTheme.muted),
                ...
                IconButton(
                    ...
                    onPressed: () {
                      final next = [...greetings]..removeAt(index);
                      onChanged(next);
                    },
              ...
              decoration: InputDecoration(
                  ...
                  suffixIcon: IconButton(
                      onPressed: () {
                        ...
                        onChanged([...greetings, controller.text.trim()]);
                        controller.clear();
                      },
