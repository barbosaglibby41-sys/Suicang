...
class PresetParser {
  ...
  static PromptPreset fromJsonText(String source, {String sourceName = '导入预设'}) {
    ...
    final extensions = json['extensions'] is Map ? Map<String, dynamic>.from(json['extensions'] as Map) : <String, dynamic>{};
    return PromptPreset(name: _string(json, 'name', fallback: sourceName), model: _string(json, 'model', fallback: _string(json, 'model_name')), temperature: _number(json, 'temperature', .8), topP: _number(json, 'top_p', .95), maxTokens: _int(json, 'max_tokens', _int(json, 'max_output_tokens', 2048)), systemPrompt: _string(json, 'system_prompt', fallback: _string(json, 'system')), source: sourceName, extensions: extensions);
  }
