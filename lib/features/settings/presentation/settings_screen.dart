        ...
        slivers: [
          ...
          SliverPadding(
            ...
            sliver: SliverList(delegate: SliverChildListDelegate([
              _Section(title: '生成配置', icon: Icons.tune_rounded, children: [_PresetCard(preset: _preset, onImport: _pickPreset)]),
              _Section(title: '世界书 / Lorebook', icon: Icons.menu_book_outlined, children: [_WorldBookCard(books: _books, onImport: _pickWorldBook)]),
...
class _WorldBookCard extends StatelessWidget {
  ...
  final VoidCallback onImport;
  @override
  Widget build(BuildContext context) => _Panel(child: Column(children: [if (books.isEmpty) const Text('还没有导入世界书', style: TextStyle(color: SuicangTheme.muted)), ...books.map((book) => ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.menu_book_outlined, color: SuicangTheme.primary), title: Text(book.name, style: const TextStyle(fontWeight: FontWeight.w700)), subtitle: Text('${book.entries.length} 个条目  ·  ${book.source}', style: const TextStyle(fontSize: 11)), trailing: const Icon(Icons.chevron_right_rounded))), const Divider(height: 20), Align(alignment: Alignment.centerLeft, child: OutlinedButton.icon(onPressed: onImport, icon: const Icon(Icons.add, size: 17), label: const Text('导入世界书')))]));
}
